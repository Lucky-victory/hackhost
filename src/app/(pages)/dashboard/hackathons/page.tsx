              ...
              {!isFetching &&
                ...
                hackathons?.map((hackathon) => (
                  <Tr key={crypto.randomUUID()}>
                    ...
                    <Td {...tdProps}>{hackathon.subtitle}</Td>
                    <Td {...tdProps}>{hackathon.description}</Td>
                    <Td {...tdProps}>
                      {Utils.formatDate(hackathon.startDate)}
                    </Td>
                    <Td {...tdProps}>{Utils.formatDate(hackathon.endDate)}</Td>
                    <Td {...tdProps}>{hackathon.slug}</Td>
                    <Td {...tdProps}>
                      {Utils.formatDate(hackathon.createdAt)}
                    </Td>
                    <Td {...tdProps}>
                      {Utils.formatDate(hackathon.updatedAt)}
                    </Td>
                    <Td {...tdProps}>{hackathon.userId}</Td>
                    <Td {...tdProps} isNumeric>
                      {hackathon.price}
                    </Td>
                    <Td {...tdProps}>{hackathon.currency}</Td>
                    <Td {...tdProps}>{hackathon.currencyCode}</Td>
                    <Td {...tdProps}>{hackathon.type}</Td>
                    <Td {...tdProps}>{hackathon.status}</Td>
                    <Td {...tdProps}>{hackathon.subStatus}</Td>
                    <Td {...tdProps}>
                      <Button
                        as={NextLink}
